...
public class S2CMessages {
        ...
        try {
            ...
            switch (clientRequest) {
                ...
                case "WRITE":
                    ...
                    System.out.println("Server " + serverId + ": Received WRITE request from Client " + clientId + " with message <" + writeRequest.message + "> for file " + writeRequest.fileName);
                    write(writeRequest);
                    break;
            }
        } catch (Exception ex) {
    ...
    public synchronized void write(WriteRequest writeRequest) {
        writeRequest.setServerId(serverId);
        server.clientWriteRequests.add(writeRequest);

        while(!writeRequest.done) {
            try {
                Thread.sleep(1);
        ...
        if (writeRequest.success) {
            ...
            System.out.println("Server " + serverId + ": Sending ABORT to Client " + clientId);
            try {
