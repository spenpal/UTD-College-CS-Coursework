...
void Renderer::nanogui_init(GLFWwindow* window)
{
	...
	gui_1->addVariable("Set Angle A", m_animation->angle_a)->setSpinnable(true);
	gui_1->addVariable("Set Angle B", m_animation->angle_b)->setSpinnable(true);
	gui_1->addButton("Rotate Local X", []() {
		m_animation->rotateLocalXAxis();
	});
	gui_1->addButton("Rotate Global Y", []() {
		m_animation->rotateGlobalYAxis();
	});
	gui_1->addButton("Reset Model", [this, gui_1]() {
		m_animation->reset();
		gui_1->refresh();
	});
